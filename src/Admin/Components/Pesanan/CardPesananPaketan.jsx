...
const CardPesananPaketan = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/order-paket/admin/all?${params.toString()}`, {
        headers: {
  ...
  const updateShippingStatus = async (orderId, newStatus) => {
    try {
      const response = await axios.patch(
        `${import.meta.env.VITE_API_URL}/api/order-paket/admin/shipping/${orderId}`,
  ...
  return (
    <div className="border rounded-md p-4 bg-white shadow mb-6">
      {/* Card Title */}
      <h2 className="text-2xl text-sky-900 font-bold mb-4">Manajemen Pemesanan Paketan</h2>
      <hr />
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {orders.map((order, index) => (
                    <tr key={order.midtrans_order_id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500">{order.nama_paket}</td>
