  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const backendImage = response.data.user.gambar ? `${import.meta.env.VITE_API_URL}${import.meta.env.VITE_API_ENDPOINT}/profile-image/${response.data.user.gambar}` : null;

        setPreviewImage(backendImage);
      } catch (error) {
        console.error('Gagal mengambil profil:', error);

        // If profile fetch fails but we have Google user info, use Google picture
    ...
    // Set Google profile picture immediately if available
    if (user && user.picture) {
      setPreviewImage(user.picture);
    }

    // Still try to fetch from backend to get the most up-to-date data
    fetchProfile();
  }, [token, user]);
  ...
  return (
    <div className="py-12">
      <div className="flex flex-col items-center justify-center">
        <img src={previewImage || 'https://placehold.co/150x150'} alt="Foto Profil" className="rounded-full h-64 w-64 object-cover" />
